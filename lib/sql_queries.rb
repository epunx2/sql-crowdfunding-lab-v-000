...
"SELECT title, SUM(pledges.amount) FROM projects INNER JOIN pledges ON pledges.project_id = project.id GROUP BY title HAVING SUM(pledges.amount) > project.funding_goal"
end
