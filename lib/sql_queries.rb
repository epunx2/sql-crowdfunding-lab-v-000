...
"SELECT title, SUM(pledges.amount) AS amount_over FROM projects INNER JOIN pledges ON pledges.project_id = project.id GROUP BY title HAVING SUM(pledges.amount) > projects.funding_goal"
end
